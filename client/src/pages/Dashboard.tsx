...
const Dashboard = () => {
  ...
  const commentOnFeed = async (feedId: number, commentContent: string) => {
    ...
    try {
      ...
      setFeedItems(feedItems.map(feed => (
        feed.id === feedId ? { ...feed, comments: [...feed.comments, response.data] } : feed
      )));
      setNewComment('');
    } catch (err) {
          ...
          <div className="flex space-x-4">
            ...
            <div className="w-3/4">
              ...
              <div className="feed-list space-y-4">
                {feedItems.length > 0 ? (
                  feedItems.map(feed => (
                    <div key={feed.id} className="feed-item p-4 border rounded shadow">
                      <p className="mb-2">{feed.content}</p>
                      <p className="text-sm text-gray-600 mb-2">Posted by: {feed.User.email}</p>
                      <p className="text-sm text-gray-600 mb-2">Likes: {feed.likes}</p>
                      ...
                      <div className="comments-section mt-4">
                        ...
                        {feed.comments.length > 0 ? (
                          feed.comments.map(comment => (
                            <div key={comment.id} className="comment p-2 border rounded mb-2">
